...
from trips.tests.bus_factories import BusFactory
...
class TestTripBusEndpoint(APITestCase):
    ...
    def test_create_bus_with_admin_roles(self):
        ...
        response = self.client.post(
            url, data=self._bus_data, format="json", HTTP_AUTHORIZATION=self._admin_header)
    ...
    def test_create_bus_without_admin_roles(self):
        url = reverse("trips:bus")
        response = self.client.post(
            url, data=self._bus_data, format="json", HTTP_AUTHORIZATION=self._header)
        response_data = response.data
